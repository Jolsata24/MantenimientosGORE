...
// V34.0 - FINAL: Traducción automática de Tipos (Inglés -> Español)
...
// Aquí defines cómo quieres que se guarde cada término
$diccionario_tipos = [
    // Origen (GLPI)       => Destino (Tu Sistema)
    'Desktop'             => 'Computadora',
    'Notebook'            => 'Laptop',
    'Laptop'              => 'Laptop',
    ...
    'Tower'               => 'Torre',
    'Low Profile Desktop' => 'PC Slim',     // O 'Bajo Perfil'
    'Docking Station'     => 'Estación Dock',
    'Server'              => 'Servidor',
...
echo "<h1>🚀 Importación GLPI V34.0 (Español)</h1>";
...
foreach ($conf_categorias as $tipo => $cfg) {
    ...
    $col_type = "'' as type_id"; 
    if (isset($cfg['fk_type']) && checkColumn($conn, $db_glpi, $cfg['tabla_glpi'], $cfg['fk_type'])) {
    ...
    while ($item = $res_main->fetch_assoc()) {
        ...
        // --- TRADUCCIÓN DEL TIPO ---
        $tipo_equipo = '-';
        if (!empty($item['type_id']) && isset($cfg['tabla_type']) && checkTable($conn, $db_glpi, $cfg['tabla_type'])) {
            $tipo_ingles = getGLPIData($conn, "SELECT name FROM $db_glpi.".$cfg['tabla_type']." WHERE id = " . $item['type_id']) ?? '-';
            
            // Verificamos si existe en el diccionario
            if (array_key_exists($tipo_ingles, $diccionario_tipos)) {
                $tipo_equipo = $diccionario_tipos[$tipo_ingles];
            } else {
                // Si no existe, lo dejamos igual (o podrías poner 'Otros')
                $tipo_equipo = $tipo_ingles;
            }
        }
