...
<head>
    <meta charset="UTF-8">
    <title>Proyectores | GORE Pasco</title>
...
<body>
    ...
    <div class="main-content">
        <div class="container-fluid">
            <div class="page-header d-flex align-items-center">
                ...
                <h2 class="titulo-seccion mb-0"><i class="fas fa-video text-purple me-2" style="color: #6f42c1;"></i> Proyectores Multimedia</h2>
            </div>
            
            <div class="card card-tabla p-3 mt-3">
                <div class="table-responsive">
                    <table id="tablaProyector" class="table align-middle w-100 table-hover table-sm">
                        ...
                        <tbody>
                            <?php
                            // FILTRO CATEGORIA = 4 (PROYECTORES)
                            $sql = "SELECT b.*, p.nombres, p.apellidos 
                                    FROM bienes b 
                                    LEFT JOIN personal p ON b.id_personal = p.id_personal
                                    WHERE b.id_categoria = 4 ORDER BY b.id_bien DESC";
                            $res = $conn->query($sql);
                            while ($fila = $res->fetch_assoc()):
                                $estado = trim($fila['estado_fisico']);
                                $clase = 'estado-regular';
                                if(strcasecmp($estado, 'Bueno') === 0) $clase = 'estado-bueno';
                                if(strcasecmp($estado, 'Malo') === 0 || strcasecmp($estado, 'Baja') === 0) $clase = 'estado-malo';
                            ?>
                            <tr>
                                <td>
                                    <div class="fw-bold text-dark small"><?php echo $fila['codigo_patrimonial']; ?></div>
                                    <small class="text-muted font-monospace" style="font-size:0.7rem"><?php echo $fila['serie'] ?: 'S/N'; ?></small>
                                </td>
                                <td>
                                ...
                                <td>
                                    <?php if ($fila['nombres']): ?>
                                        <div class="d-flex align-items-center">
                                            <div class="rounded-circle bg-light text-purple d-flex align-items-center justify-content-center border me-2" style="width:24px; height:24px; font-weight:bold; font-size:0.6rem; color: #6f42c1;">
    ...
    <script> $(document).ready(function() { $('#tablaProyector').DataTable({ language: { url: '//cdn.datatables.net/plug-ins/1.13.4/i18n/es-ES.json' }, pageLength: 10, ordering: false }); }); </script>
</body>
</html>